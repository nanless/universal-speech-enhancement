...
def energy_ratios(s_hat, s, n, eps=1e-10):
    ...
    si_sdr = 10 * np.log10(
        eps + np.linalg.norm(s_target) ** 2 / (eps + np.linalg.norm(e_noise + e_art) ** 2)
    )
    si_sir = 10 * np.log10(
        eps + np.linalg.norm(s_target) ** 2 / (eps + np.linalg.norm(e_noise) ** 2)
    )
    si_sar = 10 * np.log10(
        eps + np.linalg.norm(s_target) ** 2 / (eps + np.linalg.norm(e_art) ** 2)
    )
...
def snr_scale_factor(speech, noise, snr):
    ...
    factor = np.sqrt(speech_var / (noise_var * 10.0 ** (snr / 10.0)))
