...
The bandwidth extender upsamples an audio signal from a lower sample rate (typically
8/16/24/44.1kHz) to a full-band signal (usually 48kHz). Unlike a standard resampler, the bandwidth
extender attempts to reconstruct high frequency components that were lost/never present in the
band-limited signal.

To do this, HiFi-GAN+ first upsamples the signal to the target sample rate using bandlimited
interpolation. It then passes the signal through a stack of non-causal, non-conditional WaveNet
blocks, essentially a dilated residual convolution with a large receptive field.

The bandwidth extender is trained in the LS-GAN framework, with multiple discriminators in the time
and frequency domains. The waveform discriminators are 1D convolutional filters that are applied at
different sample rates to the output signal. The spectrogram discriminator is a 2D convolutional
filter over the log-scale mel spectrogram.

The bandwidth extender is normalized using weightnorm over all convolutional filters, which can be
removed after training to reduce computation during inference.
...
class BandwidthExtender(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, sample_rate) -> torch.Tensor:
...
class WaveNet(torch.nn.Module):
    ...
    This is a non-causal, non-conditional variant of the WaveNet architecture from van den Oord, et
    al.
    ...
    def __init__(self, sample_rate: int):
        ...
        self._convs = torch.nn.ModuleList(
            [
                ...
                for k, s, i, c, g in zip(kernel_sizes, strides, [1] + channels, channels, groups)
            ]
        )
