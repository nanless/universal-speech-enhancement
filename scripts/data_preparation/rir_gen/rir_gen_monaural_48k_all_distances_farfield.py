...
save_folder = "/root/autodl-tmp/data/rir/rirs_monaural_farfield_20240531_48k_alldistances/farfield"
# save_folder = "/data2/zhounan/data/rir/rirs_monaural_farfield_20231108_48k_alldistances/farfield"
## farfield
n_samples = 16384
n_rirs = 20000
fs = 48000
...
def gen_one_pair(i_rir):
    ...
    source_x = np.random.uniform(0.2, room_dim[0] - 0.2)
    source_y = np.random.uniform(0.2, room_dim[1] - 0.2)
    source_z = np.random.uniform(0.2, room_dim[2] - 0.2)
    source_loc = [source_x, source_y, source_z]

    while True:
        mic_loc_x = np.random.uniform(0.2, room_dim[0] - 0.2)
        mic_loc_y = np.random.uniform(0.2, room_dim[1] - 0.2)
        mic_loc_z = np.random.uniform(0.2, room_dim[2] - 0.2)
        mic_loc = [mic_loc_x, mic_loc_y, mic_loc_z]
        near_mic_dist = np.linalg.norm(np.array(mic_loc) - np.array(source_loc))
    ...
    h = rir.generate(
        ...
        r=mic_loc,  # Receiver position(s) [x y z] (m)
