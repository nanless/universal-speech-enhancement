...
from src.models.components.sgmse.util.tensors import batch_broadcast
...
class SDE(abc.ABC):
    ...
    def add_argparse_args(parent_parser):
        """Add the necessary arguments for instantiation of this SDE class to an argparse
        ArgumentParser."""
        pass
        ...
        class RSDE(oself.__class__):
            ...
            def rsde_parts(self, x, t, *args, **kwargs):
                ...
                if sde_diffusion.ndim < x.ndim:
                    sde_diffusion = sde_diffusion.view(
                        *sde_diffusion.size(), *((1,) * (x.ndim - sde_diffusion.ndim))
                    )
                score_drift = (
                    -(sde_diffusion**2) * score * (0.5 if self.probability_flow else 1.0)
                )
                diffusion = (
                    torch.zeros_like(sde_diffusion) if self.probability_flow else sde_diffusion
                )
            ...
            def discretize(self, x, t, *args, **kwargs):
                ...
                if "conditioning" in kwargs.keys() and kwargs["conditioning"] is not None:
                    rev_f = f - G**2 * score_model(
                        x, t, score_conditioning=kwargs["conditioning"], sde_input=args[0]
                    ) * (0.5 if self.probability_flow else 1.0)
                else:
                    rev_f = f - G**2 * score_model(x, t, *args) * (
                        0.5 if self.probability_flow else 1.0
                    )
...
class OUVESDE(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--sigma-min", type=float, default=0.05, help="The minimum sigma to use."
        )
        parser.add_argument(
            "--sigma-max", type=float, default=0.5, help="The maximum sigma to use."
        )
        return parser
