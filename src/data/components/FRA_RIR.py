...
def FRA_RIR(
    nsource=1, sr=16000, direct_range=[-6, 50], max_T60=0.8, alpha=0.25, a=-2.0, b=2.0, tau=0.2
):
    ...
    args:
        ...
        a, b: controlling the random pertusssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssbation added to each virtual sound source. Default: -2, 2.
    ...
    T60 = torch.FloatTensor(1).uniform_(0.05, max_T60)[0].data
    ...
    dist_range = [
        torch.linspace(1.0, velocity * T60 / direct_dist[i] - 1, image) for i in range(nsource)
    ]
    # a simple quadratic function
    dist_prob = torch.linspace(alpha, 1.0, image).pow(2)
    dist_prob = dist_prob / dist_prob.sum()
    dist_select_idx = dist_prob.multinomial(num_samples=image * nsource, replacement=True).view(
        nsource, image
    )
    ...
    reflect_max = (torch.log10(velocity * T60) - torch.log10(direct_dist) - 3) / torch.log10(
        reflect_coef + eps
    )
    ...
    # add a random perturbation based on the assumption that
