...
from src.models.components.sgmse.util.graphics import visualize_example, visualize_one
from src.models.components.sgmse.util.inference import evaluate_model
from src.models.components.sgmse.util.other import pad_spec, si_sdr_torch
...
class ScoreModel(pl.LightningModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--t_eps", type=float, default=0.03, help="The minimum time (3e-2 by default)"
        )
        parser.add_argument(
            "--num_eval_files",
            type=int,
            default=10,
            help="Number of files for speech enhancement performance evaluation during training.",
        )
        parser.add_argument(
            "--loss_type",
            type=str,
            default="mse",
            choices=("mse", "mae", "gaussian_entropy", "kristina", "sisdr", "time_mse"),
    ...
    def _step(self, batch, batch_idx):
        ...
        z = torch.randn_like(
            x
        )  # i.i.d. normal distributed with var=0.5 ---> problem: this cannot work for FreqOUVE, because is standard, and tries to match a score with a sigma which is not standard
        if std.ndim < y.ndim:
            std = std.view(*std.size(), *((1,) * (y.ndim - std.ndim)))
        sigmas = std
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_loss", loss, on_step=True, on_epoch=True, batch_size=self.data_module.batch_size
        )
        return loss
    ...
    def validation_step(self, batch, batch_idx, discriminative=False, sr=16000):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            pesq_est, si_sdr_est, estoi_est, spec, audio = evaluate_model(
                self,
                self.num_eval_files,
                spec=not self.current_epoch % VIS_EPOCHS,
                audio=not self.current_epoch % VIS_EPOCHS,
                ...
                for idx, (y, x_hat, x) in enumerate(zip(y_list, x_hat_list, x_list)):
                    if self.current_epoch == 0:
                        self.logger.experiment.add_audio(
                            ...
                            (y / torch.max(torch.abs(y))).unsqueeze(0),
                            sample_rate=sr,
                        )
                        self.logger.experiment.add_audio(
                            f"Epoch={self.current_epoch} Clean/{idx}",
                            (x / torch.max(x)).unsqueeze(0),
                            sample_rate=sr,
                        )
                    self.logger.experiment.add_audio(
                        f"Epoch={self.current_epoch} Estimate/{idx}",
                        (x_hat / torch.max(torch.abs(x_hat))).unsqueeze(0),
                        sample_rate=sr,
                    )
            ...
            if spec is not None:
                ...
                for idx, (y_stft, x_hat_stft, x_stft) in enumerate(
                    zip(y_stft_list, x_hat_stft_list, x_stft_list)
                ):
                    figures.append(
                        visualize_example(
                            torch.abs(y_stft),
...
class DiscriminativeModel(ScoreModel):
    ...
    def _loss(self, x, xhat):
        ...
        if self.loss_type == "mse":
            ...
            loss = -torch.mean(
                torch.stack([si_sdr_torch(x[i], xhat[i]) for i in range(x.size(0))])
            )
        return loss
    ...
    def enhance(self, y, **ignored_kwargs):
        with torch.no_grad():
            ...
            if self.data_module.return_time:
                ...
                Y = torch.unsqueeze(
                    self._forward_transform(self._stft((y / norm_factor).cuda())), 0
                )  # 1,D,F,T
                Y = pad_spec(Y)
            X_hat = self(Y)
            if self.dnn.FORCE_STFT_OUT:
...
class StochasticRegenerationModel(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.denoiser_net = (
            BackboneRegistry.get_by_name(backbone_denoiser)(**kwargs)
            if backbone_denoiser != "none"
            else None
        )
        ...
        self.score_net = (
            BackboneRegistry.get_by_name(backbone_score)(**kwargs)
            if backbone_score != "none"
            else None
        )
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default="mse",
            choices=("none", "mse", "mae", "sisdr", "mse_cplx+mag", "mse_time+mag"),
            help="The type of loss function to use.",
        )
        parser.add_argument(
            "--loss_type_score",
        ...
        parser.add_argument(
            "--weighting_denoiser_to_score",
            type=float,
            default=0.5,
            help="a, as in L = a * L_denoiser + (1-a) * .",
        )
        parser.add_argument(
            "--condition", default="both", choices=["noisy", "post_denoiser", "both"]
        )
        parser.add_argument("--spatial_channels", type=int, default=1)
        return parser
    ...
    def _loss(self, err, y_denoised, x):
        ...
        loss_denoiser = (
            self.loss_fn_denoiser(y_denoised, x) if self.loss_type_denoiser != "none" else None
        )
        if loss_score is not None:
            if loss_denoiser is not None:
                loss = (
                    self.weighting_denoiser_to_score * loss_denoiser
                    + (1 - self.weighting_denoiser_to_score) * loss_score
                )
            else:
                loss = loss_score
        else:
            ...
            if self.score_net is not None and not denoiser_only:
                ...
                if sampler_type == "pc":
                    ...
                    sampler = self.get_ode_sampler(
                        Y_denoised, N=N, conditioning=score_conditioning, **kwargs
                    )
                else:
                    print(f"{sampler_type} is not a valid sampler type!")
                sample, nfe = sampler()
