            ...
            if progressive != "none":
                if i_level == num_resolutions - 1:
                    ...
                    if progressive == "output_skip":
                        ...
                        modules.append(
                            conv3x3(in_ch, self.total_channels, bias=True, init_scale=init_scale)
                        )
                        pyramid_ch = self.total_channels
                    elif progressive == "residual":
        ...
        for chan in range(self.spatial_channels):
            x_chans.append(
                torch.cat(
                    [
                        torch.cat(
                            [x[:, [chan + in_chan], :, :].real, x[:, [chan + in_chan], :, :].imag],
                            dim=1,
                        )
...
class NCSNpp12M(NCSNpp):
    ...
    def __init__(self, **kwargs):
        super().__init__(
            nf=96, ch_mult=(1, 2, 2, 1), num_res_blocks=1, attn_resolutions=(0,), **kwargs
        )
