...
def visualize_example(
    ...
    im = ax.pcolormesh(
        frames,
        freqs,
        20 * np.log10(0.1 * mix + EPS_graphics),
        vmin=vmin,
        vmax=vmax,
        shading="auto",
        cmap="magma",
    ...
    ax.pcolormesh(
        frames,
        freqs,
        20 * np.log10(0.1 * estimate + EPS_graphics),
        vmin=vmin,
        vmax=vmax,
        shading="auto",
        cmap="magma",
    ...
    ax.pcolormesh(
        frames,
        freqs,
        20 * np.log10(0.1 * target + EPS_graphics),
        vmin=vmin,
        vmax=vmax,
        shading="auto",
        cmap="magma",
    ...
    if return_fig:
        ...
        plt.savefig(
            os.path.join(spec_path, f"spectro_{idx_sample}_epoch{epoch}{name}.png"),
            bbox_inches="tight",
        )
        plt.close()
...
def visualize_one(estimate, spec_path, name="", sample_rate=16000, hop_len=128, raw=True):
    ...
    im = plt.pcolormesh(
        frames,
        freqs,
        20 * np.log10(estimate.abs() + EPS_graphics),
        vmin=vmin,
        vmax=vmax,
        shading="auto",
        cmap="magma",
